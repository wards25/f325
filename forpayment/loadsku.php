...
// query
$order_query = mysqli_query($conn,"SELECT * FROM dbraw WHERE f325number='$f325number' AND paymentstatus='$status' ");
while ($fetch_order = mysqli_fetch_array($order_query))
{
	?>
	<tr class="tbl-order-detail-tr" rowid="<?php echo $fetch_order['id']; ?>">
		...
		<td class="tbl-order-detail-td10">
			<?php echo $fetch_order['quantity']; ?>
		</td>
		<td class="tbl-order-detail-td11">
			<?php echo $fetch_order['rcvdqty']; ?>
		</td>
		<td class="tbl-order-detail-td5">
			<?php echo $quantity = ($fetch_order['rcvdqty']-$fetch_order['quantity']); ?>
		</td>
