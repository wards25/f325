...
if (strlen($_POST['company']) >= 1)
{
	...
	$query .= " WHERE company='$company' AND (status='FOR BILLED' OR status='FOR BILLING' OR status='PAID' OR status='CANCELLED')";
}
else
{
	// company
	...
	$query .= " WHERE (".$company.") AND (status='FOR BILLED' OR status='FOR BILLING' OR status='PAID' OR status='CANCELLED') ";
}
