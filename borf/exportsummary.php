...
$export_query = "SELECT * FROM dbf325number WHERE status='PRINTED' AND process='APPS' AND emaildate='$nameexport' ";
...
$export_query .= "AND process='APPS' ORDER BY vendor,brcode ASC";
...
while ($fetch_exportcsv = mysqli_fetch_array($exportcsv_query))
{
	...
	if (is_array($fetch_census)) {$branchname = $fetch_census['branchname'];$region = $fetch_census['region'];}else{$branchname = "For Fill-Up";$region = "For Fill-Up";}
	...
	if (is_array($fetch_company)) {$vendor = $fetch_company['nickname'];}else{$vendor='For Fill-Up';}
