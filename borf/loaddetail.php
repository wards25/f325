...
$detail_query = mysqli_query($conn,"SELECT * FROM dbborflist WHERE id='$id' ");
$fetch_detail = mysqli_fetch_array($detail_query);

$borfnumber = $fetch_detail['borfnumber'];
$bms = $fetch_detail['userprocessed'];
$f325number = $fetch_detail['f325number'];
$f325date = $fetch_detail['f325date'];
$emaildate = $fetch_detail['emaildate'];

// get db status of f325
$db_query = mysqli_query($conn,"SELECT status,f325number FROM dbf325number WHERE f325number='$f325number' ");
$fetch_db = mysqli_fetch_array($db_query);

if (is_array($fetch_db))
{
	$dbstatus = $fetch_db['status'];
}
else
{
	$dbstatus = "NONE";
