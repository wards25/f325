...
header('Content-Disposition: attachment; filename="Pull Out Report as of '.date('m-d-Y').'.csv"');
//no cache
...
if (isset($_POST['select_location']))
{
	...
	{
		$raw_query = mysqli_query($conn,"SELECT * FROM dbraw WHERE statusout='PULL-OUT' ");
	}
	else
	{
		$location = $_POST['select_location'];
		$raw_query = mysqli_query($conn,"SELECT * FROM dbraw WHERE location='$location' AND statusout='PULL-OUT' ");
	}
